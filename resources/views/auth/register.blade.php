<x-guest-layout>
    <form method="POST" action="{{ route('register') }}">
        ...
        <div class="mt-4">
            ...
            <x-text-input id="phone" class="block mt-1 w-full" type="text" name="phone" :value="old('phone')" required placeholder="+256700000000" />
        ...
        <div class="mt-4">
            ...
            <select id="role" name="role" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm" required>
                ...
                <option value="landlord" {{ old('role') === 'landlord' ? 'selected' : '' }}>{{ __('Landlord (I want to list properties)') }}</option>
                <option value="tenant" {{ old('role') === 'tenant' ? 'selected' : '' }}>{{ __('Tenant (I want to find properties)') }}</option>
            </select>
